...
| M1 | **Read locks** + `grant_contiguous_readers` batching + derived state. Concurrent co-holding, single-release batch grant, stop-at-queued-writer (§6.2), `max_reader_batch` cap, state accuracy (via `inspect`) — all tested. | ✅ done |
...
| M4 | **Recovery**: lazy cleanup + self-wake + optional auto-detected keyspace-expiry subscriber. The `writer` key is PEXPIRE'd to its lease, so a crashed writer self-expires and fires `__keyevent@*__:expired`; the subscriber runs expire_and_grant out-of-band (readers recover via self-wake). Never CONFIG SET; silent fallback to self-wake. | ✅ done |
...
| M6 | **Observability**: pluggable `Metrics` + `Tracer` sinks (no OTel/prom hard dep) emitting the §18 set (acquire/wait/held/timeouts/extend/release/lock-lost/fencing/blocking-in-use); read-only `inspect(resource)` (no-writes function); blocking-connection gauge + `blockingConnectionsInUse`. | ✅ done |
